...
public class JoueursTest {
    ...
    public void testAcheterPropriete() {
        ...
        assertEquals(joueur, propriete.getProprietaire(),
                "Le joueur devrait être le propriétaire de la propriété après l'achat");
        ...
        Cases proprieteChere = new Cases(2, "Test", 2000, "Test", loyer, 50, 100);
        joueur.acheterPropriete(proprieteChere);

        // Vérifiez que le joueur n'a pas acheté la propriété
        assertFalse(joueur.getProprietes().contains(proprieteChere),
                "Le joueur ne devrait pas posséder la propriété après un achat échoué");
        assertNull(proprieteChere.getProprietaire(),
                "La propriété ne devrait pas avoir de propriétaire après un achat échoué");
    }
    ...
    public void testEnvoyerEnPrison() {
        ...
        assertEquals(10, joueur.getPlacement(),
                "Le joueur devrait être à la position 10 après avoir été envoyé en prison");
    }
}